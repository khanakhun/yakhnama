"""FastAPI dependencies of the impact claims HTTP API: services and the ``Actor``.

The composition root stores the ``Container`` on ``app.state.container``; this
package reads it through ``ImpactClaimsApiServices`` and never imports
...
class ImpactClaimsApiServices(Protocol):
    """What the impact claims router reads from the composition root.
    ...
    @property
    def impact_claim_handler_dependencies(self) -> ImpactClaimHandlerDependencies:
        """Return the ports every impact claim command handler is built from."""
        ...

    @property
    def event_impacts_queries(self) -> EventImpactsQueryService:
        """Return the authorised event impacts query service."""
        ...


def get_impact_claims_services(request: Request) -> ImpactClaimsApiServices:
    ...
    Returns:
        The container, seen through ``ImpactClaimsApiServices``.

    Raises:
        RuntimeError: If the app was not built by ``create_app``; a wiring bug.
    """
    services: ImpactClaimsApiServices | None = getattr(
...
Services = Annotated[ImpactClaimsApiServices, Depends(get_impact_claims_services)]
...
async def _actor_for(principal: Principal, services: ImpactClaimsApiServices) -> Actor:
