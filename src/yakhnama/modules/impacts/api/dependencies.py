"""FastAPI dependencies of the impacts HTTP API.

The composition root stores the ``Container`` on ``app.state.container``. This
package reads it through the ``ImpactsApiServices`` protocol instead of importing
...
class ImpactsApiServices(Protocol):
    """What the impacts router reads from the composition root.

    Implements: Dependency Injection (provider port bound in the composition root).
    """

    @property
    def impact_metric_query_service(self) -> ImpactMetricQueryService:
        """Return the impact metric query service."""
        ...


def get_impacts_services(request: Request) -> ImpactsApiServices:
    ...
    Returns:
        The container, seen through ``ImpactsApiServices``.

    Raises:
        RuntimeError: If the app was not built by ``create_app``; a wiring bug, so a
            server error rather than a domain error.
    """
    services: ImpactsApiServices | None = getattr(request.app.state, "container", None)
    if services is None:
...
Services = Annotated[ImpactsApiServices, Depends(get_impacts_services)]
