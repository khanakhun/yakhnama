"""FastAPI dependencies of the events HTTP API: services and the acting ``Actor``.

The composition root stores the ``Container`` on ``app.state.container``; this
package reads it through ``EventsApiServices`` and never imports
...
class EventsApiServices(Protocol):
    """What the events router reads from the composition root.
    ...
    @property
    def event_handler_dependencies(self) -> EventHandlerDependencies:
        """Return the ports every events command handler is built from."""
        ...

    @property
    def event_queries(self) -> EventRecordQueryService:
        """Return the authorised events query service."""
        ...
...
def get_events_services(request: Request) -> EventsApiServices:
    ...
    Returns:
        The container, seen through ``EventsApiServices``.

    Raises:
        RuntimeError: If the app was not built by ``create_app``; a wiring bug.
    """
    services: EventsApiServices | None = getattr(request.app.state, "container", None)
    if services is None:
...
async def _actor_for(principal: Principal, services: EventsApiServices) -> Actor:
