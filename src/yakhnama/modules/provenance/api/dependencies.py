"""FastAPI dependencies of the provenance HTTP API: services and the acting ``Actor``.

The composition root stores the ``Container`` on ``app.state.container``; this
package reads it through ``ProvenanceApiServices`` and never imports
...
class ProvenanceApiServices(Protocol):
    """What the provenance router reads from the composition root.
    ...
    @property
    def source_registrar(self) -> SourceRegistrar:
        """Return the register-source use case."""
        ...

    @property
    def source_queries(self) -> AuthorisedSourceQueryService:
        """Return the authorised source query service."""
        ...


def get_provenance_services(request: Request) -> ProvenanceApiServices:
    ...
    Returns:
        The container, seen through ``ProvenanceApiServices``.

    Raises:
        RuntimeError: If the app was not built by ``create_app``; a wiring bug.
    """
    services: ProvenanceApiServices | None = getattr(
...
Services = Annotated[ProvenanceApiServices, Depends(get_provenance_services)]
...
async def _actor_for(principal: Principal, services: ProvenanceApiServices) -> Actor:
